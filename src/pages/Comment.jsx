...
const Comment = () => {
 ...
 const satisfactionLevels = [
  { value: 1, emoji: '😡', description: 'በጣም እልረካሁም' },
  { value: 2, emoji: '😟', description: 'አልረካሁም' },
  ...
  <NavLayout>
   ...
   <div className='sm:flex gap-10 mb-5 mt-10 md:px-20'>
    <div className='sm:w-full p-6 bg-white rounded-md shadow-md'>
     <div className="text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700 mb-10">
      <ul className="flex flex-wrap -mb-px">
       ...
       <li className="me-2 cursor-pointer" onClick={() => setCommentType("touser")}>
        <a className={`inline-block p-4 ${commentType === 'touser' ? "text-blue-600 border-b-2 border-blue-600" : ""} rounded-t-lg active`}>{t('commentforservice')}</a>
       </li>
      </ul>
     </div>
     ...
     {commentType === 'touser' && (
      <>
       <label htmlFor="user" className="block text-sm font-medium text-gray-700">{t('commentp')}</label>
     ...
     <form onSubmit={handleSubmit} className="">
      {commentType === 'touser' && (
       <div className="mb-4">
        <label htmlFor="window_number" className="block text-sm font-medium text-gray-700">{t('commentt')}</label>
        <input
         type="text"
         disabled
         id="window_number"
      ...
      <div className="mb-4">
       <label htmlFor="level_of_satisfaction" className="block text-sm font-medium text-gray-700">{t('comments')}</label>
      ...
      {commentType === 'touser' && <div className="mb-4">
       <label htmlFor="ticket" className="block text-sm font-medium text-gray-700">{t('commentw')}</label>
       <input
        type="text"
        id="ticket"
        value={ticket}
        ...
        className="bg-gray-200 appearance-none border-2 border-white rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500"
        placeholder={t('commentw1')}
       />
      </div>}

      <div className="mb-4">
       <label htmlFor="content" className="block text-sm font-medium text-gray-700">{t('commentf')}</label>
       <textarea
        id="content"
        value={content}
